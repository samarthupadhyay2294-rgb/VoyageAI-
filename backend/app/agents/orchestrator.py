...
class TripOrchestrator:
    ...
    async def generate_trip_plan(self, trip: Dict[str, Any], regenerate: bool = False) -> Dict[str, Any]:
        ...
        try:
            ...
            if existing_plan:
                if regenerate:
                    # Update existing plan during regeneration
                    await self.trip_repo.update_trip_plan(trip_id, user_id, trip_plan_data)
                    logger.info(f"Updated existing trip plan for trip {trip_id}")
                else:
                    # Don't overwrite existing plan unless explicitly regenerating
                    logger.info(f"Trip plan already exists for trip {trip_id}, skipping creation")
                    trip_plan_data = existing_plan  # Return existing plan
            else:
                # Create new plan
                plan_data = {
                    "trip_id": trip_id,
                ...
                logger.info(f"Created new trip plan for trip {trip_id}")
